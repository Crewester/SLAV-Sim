...
public class FourWayLightEntranceCheck : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            roundabout.GetComponent<FourWayLights>().Box1 = true;
        }
        if (other.gameObject.layer == 7 && gameObject.name == "BoxEnt2")
        {
            roundabout.GetComponent<FourWayLights>().Box2 = true;
        }
        if (other.gameObject.layer == 7 && gameObject.name == "BoxEnt3")
        {
            roundabout.GetComponent<FourWayLights>().Box3 = true;
        }
        if (other.gameObject.layer == 7 && gameObject.name == "BoxEnt4")
        {
            roundabout.GetComponent<FourWayLights>().Box4 = true;
        }
        active = false;
    }
}
